...
std::string getCoordinates(const std::string& location, const std::string& apiKey) {
    ...
    curl = curl_easy_init();
    if (curl) {
        // Construct the GoMaps Geocoding API URL
        std::string url = "https://maps.gomaps.pro/maps/api/geocode/json?address=" + encodedLocation + "&key=" + apiKey;
...
std::string getDistance(const std::string& originCoords, const std::string& destinationCoords, const std::string& apiKey) {
    ...
    curl = curl_easy_init();
    if (curl) {
        // Construct the GoMaps Distance Matrix API URL
        std::string url = "https://maps.gomaps.pro/maps/api/distancematrix/json?origins=" + originCoords +
